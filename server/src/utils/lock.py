...
class MRedisLock:
    ...
    def m_acquire(self, suffix_ls: list[str]) -> tuple[bool, str]:
        ...
        while True:
            ...
            ex_release = any(_release_by_ex(self._redis, self.ex, curr_time=curr_time, key=k) for k in mapping.keys())
            if not ex_release:
...
def _release_by_ex(redis_client: Any, ex: int, curr_time: int, key: str) -> bool:
    """
    判断key是否超时，超时则解锁
    成功解锁了超时的key时返回True
    """
    val = redis_client.get(key)
    if val and ((curr_time - int(val.decode('utf-8'))) >= ex):
